...
class RateLimiter:
    ...
    async def wait_if_needed(self, estimated_tokens: int = 0) -> Tuple[bool, str]:
        ...
        async with self._lock:
            ...
            current_minute_tokens = sum(self.token_usage)
...
class TokenManager:
    ...
    def truncate_context(self, context_text: str, max_tokens: int) -> str:
        ...
        # Truncate by chunks/paragraphs first
        ...
        if not truncated_chunks:
            ...
            chars_per_token = len(context_text) / current_tokens
    ...
    def optimize_messages_for_limits(self, messages: List[Dict[str, str]], max_context_tokens: int = None) -> List[Dict[str, str]]:
        ...
        # Strategy 1: Truncate user context in last message
        if len(optimized_messages) > 1:
            last_message = optimized_messages[-1]
            if last_message.get("role") == "user" and len(last_message.get("content", "")) > 1000:
                # Look for context section
                content = last_message["content"]
                if "Relevante Dokumente:" in content:
                    parts = content.split("Relevante Dokumente:", 1)
                    if len(parts) == 2:
                        question_part = parts[0]
                        context_part = parts[1]
                        
                        # Calculate available tokens for context
                        base_tokens = self.count_message_tokens(optimized_messages[:-1])
                        question_tokens = self.count_tokens(question_part)
                        available_for_context = max_context_tokens - base_tokens - question_tokens - 100
                        
                        if available_for_context > 200:
                            truncated_context = self.truncate_context(context_part, available_for_context)
                            optimized_messages[-1]["content"] = question_part + "Relevante Dokumente:" + truncated_context
        ...
        if current_tokens > max_context_tokens and len(optimized_messages) >= 2:
            ...
            if last_message.get("role") == "user":
                ...
                optimized_messages[-1]["content"] = self.truncate_context(
...
class RAGContextManager:
    ...
    async def retrieve_context(self, query: str, max_tokens: int = None) -> Optional[RetrievedContext]:
        ...
        try:
            ...
            original_token_count = self.token_manager.count_tokens(context_text)
            
            # Apply token truncation if needed
            truncated = False
            final_token_count = original_token_count
            
            if max_tokens and self.config.context_truncation_enabled:
                if original_token_count > max_tokens:
                    context_text = self.token_manager.truncate_context(context_text, max_tokens)
    ...
    def _combine_context_chunks(self, chunks: List[Dict[str, Any]]) -> str:
        """Kombiniert context chunks zu einem Text mit Templates"""
        ...
        for chunk in chunks:
            formatted_chunk = self.template_manager.render_template(
                "context_chunk",
                source=chunk["source"],
                score=chunk["score"],
...
class BUProcessorChatbot:
    ...
    async def _call_openai_api(self, messages: List[Dict[str, str]]) -> Dict[str, Any]:
        """Ruft OpenAI API mit Retry-Logic auf"""
