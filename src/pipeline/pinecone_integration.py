...
class AsyncPineconeManager:
    """Enhanced Async Pinecone Client Manager"""
    ...
    def __init__(self, config: AsyncPineconeConfig, metrics: PineconeMetrics):
        ...
        self.batch_times = deque(maxlen=100)  # Last 100 batch times
        self.throughput_history = deque(maxlen=50)  # Last 50 throughput measurements
...
async def demo_enhanced_pinecone_integration():
    ...
    if not PINECONE_AVAILABLE:
        console.print("❌ [bold red]Pinecone not available.[/bold red] Install with: pip install pinecone-client")
        return
