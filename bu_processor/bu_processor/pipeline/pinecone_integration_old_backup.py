...
from ..integrations.pinecone_manager import (
    ...
    VectorSearchResult as SimplifiedVectorSearchResult,
...
class PineconeManager:
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Extract relevant parameters and ignore legacy ones
        clean_kwargs = {
            'api_key': api_key,
    ...
    def delete_vectors(self, vector_ids: List[str], **kwargs) -> Dict[str, Any]:
        ...
        return self._manager.delete_vectors(vector_ids, **kwargs)
...
def PineconeManagerAlias(**kwargs) -> PineconeManager:
    ...
    return get_pinecone_manager(**kwargs)
...
class PineconeManager:
    ...
    def get_index_dimension(self):
        """Get the dimension of the Pinecone index for preflight checks"""
        if self.stub_mode:
            # Return consistent test dimension in stub mode
            return 1536  # Standard OpenAI embedding dimension
        
        if not self._client:
            return None
            
        try:
            # v3 SDK: describe_index
            info = self._client.describe_index(self.index_name)
            return info.dimension
        except Exception as e:
            logger.warning("Could not get index dimension", error=str(e))
            return None
