...
def demo_enhanced_semantic_simhash():
    """Demonstriert die erweiterte semantische Deduplication v2.0"""
    
    print("🚀 ERWEITERTE SEMANTISCHE SIMHASH DEDUPLICATION v2.0")
    print("=" * 60)
    print("✨ NEUE FEATURES:")
    print("   • Corpus-weite TF-IDF-ähnliche Term-Gewichtung")
    print("   • Intelligente Fachbegriff-Erkennung für Versicherungsdomäne")
    print("   • Gewichtete kombinierte Ähnlichkeitsmetrik (70% Hamming + 30% Features)")
    print("   • Erweiterte semantische Feature-Extraktion")
    print("   • Performance-optimierte Cluster-Bildung")
    print()
    
    # Test-Chunks mit semantischen Duplikaten
    test_chunks = [
        HierarchicalChunk(
            id="chunk_1", 
            text="Die Berufsunfähigkeitsversicherung bietet umfassenden Schutz bei Verlust der Arbeitsfähigkeit durch Krankheit oder Unfall. Die monatliche Rente beträgt bis zu 3.000 € und wird lebenslang gezahlt.",
        ...
        HierarchicalChunk(
            id="chunk_2",
            text="Eine Berufsunfähigkeitsversicherung gewährt vollständigen Schutz, wenn die Erwerbsfähigkeit durch Erkrankung oder Verletzung verloren geht. Die monatliche BU-Rente kann bis zu 3000 Euro betragen.",
            content_type=ContentType.LEGAL_TEXT,
            heading_level=2,
            token_count=26
        ),
        HierarchicalChunk(
            id="chunk_3",
            text="Die BU-Versicherung schützt vor finanziellen Einbußen bei beruflicher Arbeitsunfähigkeit aufgrund gesundheitlicher Probleme. Monatliche Leistung: maximal 3.000€.",
            content_type=ContentType.LEGAL_TEXT,
            heading_level=1,
            token_count=22
        ),
        HierarchicalChunk(
            id="chunk_4",
            text="Bei einer abstrakten Verweisung prüft der Versicherer, ob der Versicherungsnehmer noch irgendeiner Tätigkeit nachgehen kann. Dies erfolgt ohne Berücksichtigung der konkreten Arbeitsmarktlage.",
            content_type=ContentType.LEGAL_TEXT,
            heading_level=2,
            token_count=28
        ),
        HierarchicalChunk(
            id="chunk_5",
            text="Der Ausschluss von Vorerkrankungen gilt nicht, wenn die Krankheit nicht in den Gesundheitsfragen erwähnt wurde. Wartezeiten entfallen bei Unfällen vollständig.",
            content_type=ContentType.LEGAL_TEXT,
            heading_level=2,
            token_count=23
        )
    ...
    for i, chunk in enumerate(test_chunks, 1):
        print(f"   {i}. [{chunk.content_type.value}] {chunk.text[:60]}...")
    
    # Erweiterte semantische Deduplication
    engine = EnhancedDeduplicationEngine()
    deduplicated_chunks, report = engine.deduplicate_chunks_semantically(test_chunks)
    ...
    print(f"   Similarity Threshold: {perf_metrics['similarity_threshold']}")
    print(f"   Hamming Threshold: {perf_metrics['hamming_threshold']}")
    print(f"   Processing Method: {perf_metrics['processing_method']}")
    
    if report['cluster_details']:
        print(f"\n📝 CLUSTER DETAILS:")
        for cluster in report['cluster_details']:
            print(f"   • {cluster['cluster_id']}: {cluster['total_chunks']} chunks, Representative: {cluster['representative']}")
    
    print(f"\n📝 VERBLIEBENE CHUNKS:")
    for i, chunk in enumerate(deduplicated_chunks, 1):
        print(f"   {i}. [{chunk.content_type.value}] {chunk.text[:70]}...")
    
    print(f"\n✅ VERBESSERUNGEN v2.0:")
    improvements = [
        "✅ Corpus-weite TF-IDF-ähnliche Term-Gewichtung",
        "✅ Intelligente Fachbegriff-Erkennung (Versicherungsdomäne)", 
        "✅ Gewichtete kombinierte Ähnlichkeitsmetrik (70/30)",
        "✅ Erweiterte semantische Feature-Extraktion",
        "✅ Graph-basierte Cluster-Verfeinerung",
        "✅ Multi-Faktoren Term-Gewichtung",
